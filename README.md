...
ببین این فایل های صورت پروژه درس (برنامه نویسی شی گرا)) من هستش ، این چهار فایل رو به طور کامل و دقیق بخون و لطفا برام به زبون انگلیسی و به شکلی زیبا ، جذاب و جدید و کاملا مرتبط با این فایل ها ، یک Readme.md کامل برای گیت هاب برام آماده کن

همچنین به من بگو ، طبق این فایل ها ، عنوان و شرح رو چی بنویسیم داخل گیت هاب و چه تگ هایی رو بزارم ؟
...
* Flexible card placement controls: **Drag-and-Drop**, **Click-to-Place**, or **Coordinate Text Input** .
...
* **Interactive Level Map:** Visual stage roadmap tracking player progression through 5 distinct tiers .
...
```bash
git clone [https://github.com/](https://github.com/)<your-username>/city-wars-tokyo-reign.git
...
🎨 UI Showcase & Polish
    ...
    Adaptive Audio Engine: Dynamic background music (BGM) selector, sound effect volume control, and mute toggle .  
...
README
 MD 
۱. مشخصات پیشنهادی برای گیت‌هاب (Title, Description & Tags)

برای اینکه ریپازیتوری شما در گیت‌هاب بسیار حرفه‌ای، جذاب و استاندارد باشد، بخش‌های زیر را به این صورت تنظیم کنید:
🔹 نام ریپازیتوری (Repository Name)

یکی از گزینه‌های زیر را انتخاب کنید:

    گزینه اول (پیشنهادی): city-wars-tokyo-reign

    گزینه دوم: city-wars-javafx-oop

    گزینه سوم: tokyo-reign-card-game

🔹 عنوان / تیتر اصلی (Repository Headline / Subtitle)

    City Wars: Tokyo Reign — A Cyberpunk Tactical Card Battler with JavaFX & OOP Architecture

🔹 شرح خلاصه در بخش About (Repository Description)

    A feature-rich tactical card duel game inspired by "City Wars: Tokyo Reign", developed in Java & JavaFX. Features 1v1 PvP, Wager betting, 5-stage AI Campaign with custom Boss Fights, real-time animated Timeline combat, Hit-Bar mechanics, dynamic Marketplace, and an in-game Admin CMS.

      

🔹 تگ‌ها و تاپیک‌ها (GitHub Topics / Tags)

تگ‌های زیر را در بخش About -> Topics ریپازیتوری خود قرار دهید تا بازدید و سئوی گیت‌هاب شما به حداکثر برسد:
Plaintext

java, javafx, object-oriented-programming, oop, game-development, card-game, tactical-game, mvc-architecture, scenebuilder, fxml, cyberpunk, turn-based-strategy, ai-bot, sharif-university, desktop-app

۲. محتوای کامل فایل README.md

فایل کامل README.md به زبان انگلیسی، همراه با بج‌های اختصاصی (Badges)، طراحی مدرن، جداول مشخصات اسپل‌ها و معماری نرم‌افزاری ایجاد و آماده دانلود شده است[cite: 3, 4]. متن کامل آن به شرح زیر است:
Markdown
...
**City Wars: Tokyo Reign** is an advanced tactical card battle game built from the ground up as a comprehensive term project for the **Object-Oriented Programming (OOP)** course at **Sharif University of Technology**[cite: 3, 4]. 

The project demonstrates clean OOP principles, design patterns, separation of concerns (MVC architecture), interactive graphical user interfaces (JavaFX + Scene Builder), persistent data management, real-time animation loops, and intelligent bot algorithms[cite: 1, 3, 4].

Originally engineered as a robust command-line interface (Phase 1) and subsequently transformed into an immersive, cyberpunk-themed desktop application (Phase 2), the game delivers strategic duel mechanics, multi-mode gameplay, an in-game card marketplace, player progression systems, and complete administrator capabilities[cite: 1, 3, 4].
